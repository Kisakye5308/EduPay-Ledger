    ...
    <div className="overflow-x-auto">
      <table className="w-full text-left border-collapse min-w-[500px]">
        ...
        <tbody className="text-sm">
          {data.map((row) => (
            <tr key={row.class} className="border-t border-slate-100 dark:border-slate-800">
              ...
              {allStreams.map(streamName => {
                ...
                return (
                  <td key={streamName} className="p-1">
                    ...
                    >
                      <span className="text-sm">{formatAmount(stream.value)}</span>
                      <span className="text-[9px] opacity-70">{stream.studentCount} students</span>
                    </div>
                  </td>
                );
              })}
        ...
        <tfoot>
          <tr className="border-t-2 border-slate-200 dark:border-slate-700">
            <td className="py-3 pr-4 font-bold text-primary dark:text-white">TOTAL</td>
            {allStreams.map(streamName => {
              ...
              return (
                <td key={streamName} className="py-3 px-2 text-center font-bold text-primary dark:text-white">
                  {formatAmount(total)}
                </td>
              );
            })}
            <td className="py-3 pl-4 text-right font-bold text-lg text-red-600 dark:text-red-400">
              {formatAmount(data.reduce((sum, row) => sum + row.totalArrears, 0))}
            </td>
          </tr>
        </tfoot>
      </table>
...
function ActivityFeed({ activities, maxItems = 6, onViewAll }: ActivityFeedProps) {
      ...
      <div className="space-y-4 overflow-y-auto flex-1 pr-2">
        {displayedActivities.length === 0 ? (
          ...
          displayedActivities.map((activity) => (
            ...
            >
              <div className={`size-9 ${activity.iconBg} ${activity.iconColor} rounded-full flex items-center justify-center shrink-0`}>
      ...
      <div className="mb-8 flex flex-col lg:flex-row lg:justify-between lg:items-end gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            Real-time fee collection status • {stats.activeStudents} active students
          </p>
        </div>
        ...
        <div className="col-span-12 lg:col-span-8 space-y-6">
          ...
          <Card>
            <div className="flex items-center justify-between mb-6">
              ...
              <Link href="/reports" className="text-sm text-primary dark:text-blue-400 hover:underline">
                View Details →
              </Link>
            </div>
          ...
          <Card>
            ...
            <div className="grid grid-cols-1 gap-3 mt-4">
              ...
              <Link href="/reports?type=receipts">
                ...
                >
                  <div className="flex items-center gap-3">
                    <span className="material-symbols-outlined bg-slate-200 dark:bg-slate-700 p-2 rounded-md">receipt_long</span>
              ...
              <Link href="/arrears">
                <Button
                  ...
                  icon={<span className="material-symbols-outlined text-red-400 group-hover:translate-x-1 transition-transform">chevron_right</span>}
                  iconPosition="right"
                >
                  <div className="flex items-center gap-3">
                    <span className="material-symbols-outlined bg-red-100 dark:bg-red-900/30 text-red-600 p-2 rounded-md">warning</span>
          ...
          <Card>
            ...
            <div className="space-y-3 mt-4">
              <div className="flex justify-between items-center py-2 border-b border-slate-100 dark:border-slate-800">
                ...
                <span className="font-bold text-primary dark:text-white">{stats.totalStudents}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-slate-100 dark:border-slate-800">
                ...
                <span className="font-bold text-emerald-600">{stats.fullyPaidStudents}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-slate-100 dark:border-slate-800">
                <span className="text-slate-600 dark:text-slate-400 flex items-center gap-2">
                ...
                <span className="font-bold text-amber-600">{stats.partialStudents}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-slate-100 dark:border-slate-800">
                <span className="text-slate-600 dark:text-slate-400 flex items-center gap-2">
                ...
                <span className="font-bold text-red-600">{stats.overdueStudents}</span>
              </div>
              <div className="flex justify-between items-center py-2">
                ...
                <span className="font-bold text-slate-600">{stats.noPaymentStudents}</span>
              </div>
            </div>
          </Card>
